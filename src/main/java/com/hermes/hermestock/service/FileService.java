...
public class FileService {
    ...
    public List<TradeLog> readFile4Mig(String fileName, Integer fileOrder, String targetDate) {
        List<TradeLog> tradeLogs = new ArrayList<>();
        File csv = new File(fileName);
        Common common = new Common();
        BufferedReader br = null;
     ...
     * @return
     */
    public List<RateLog> readFile(String fileName, String targetDate){
        ...
        try {
            ...
            br = new BufferedReader(new InputStreamReader(new FileInputStream(csv), "iso-8859-1"));
            if(targetDate.equals("")) targetDate = LocalDate.now().format(dtf).toString();
            while ((line = br.readLine()) != null) { // readLine()은 파일에서 개행된 한 줄의 데이터를 읽어온다.
                ...
                String[] values = readLine.split(","); // 파일의 한 줄을 ,로 나누어 배열에 저장 후 리스트로 변환한다.
                if(values[0].equals("종목코드")) continue; // 타이틀 라인인 경우 pass , 메세지의 start, next 에서 1 빼야 함
                rateLog.setRateLog(values[0].replace("\"", ""), common.isPositive(values[4].replace("\"", "")), targetDate);
